...
\begin{itemize}
	...
	\item 两条\texttt{ADD}计算\texttt{A+B}和\texttt{B+C}，再用\texttt{SW}写回结果。中间加入无关指令有意地拉开距离。
...
观察与分析：
\begin{itemize}
	\item 执行周期总数为14，出现RAW停顿4次（2次为load停顿，占所有RAW停顿50\%）。
	\item 周期图中可见\texttt{ADD}和\texttt{SW}前后存在明显停顿，符合load-use冲突机理。
\end{itemize}
