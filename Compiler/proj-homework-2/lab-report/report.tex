...
\author{\authname：\underline{\hspace{3cm}} \quad \authid：\underline{\hspace{3cm}} \quad \class：\underline{\hspace{3cm}}}
\date{\today}
...
\begin{itemize}
  ...
  \item \textbf{实施代码优化}：实现三个窥孔优化遍（冗余跳转消除、存储-加载转发、自赋值消
        除），在不改变语义的前提下减少指令数量。
  \item \textbf{运用 AI 辅助开发}：借助 AI 工具完成编译器各阶段的设计与代码实现，提升工程
        效率，并对生成内容进行人工审查与验证。
\end{itemize}
