...
% ===== 模板配置 - 请根据实验要求修改 =====
\newcommand{\reporttitle}{实验五：SLR引导的语义分析框架实现}
\newcommand{\subtask}{SLR(1)语义动作 + AST + 符号表}
...
本实验的目标如下：
\begin{itemize}
	\item 目标一：\textbf{在 SLR(1) 语法分析基础上集成语义动作并维护属性栈}
	\item 目标二：\textbf{构建抽象语法树（AST）并输出可视化结构}
	\item 目标三：\textbf{维护符号表与作用域，完成类型检查与语义错误检测}
\end{itemize}
...
\begin{itemize}
	...
	\item 编程语言：\textbf{C}
	\item 编译器：\textbf{GCC}
	\item 其他关键工具：\textbf{终端与文本编辑器}
\end{itemize}

\subsection{输入与预处理}
输入包括两部分：
\begin{itemize}
	\item SLR(1) 分析表：由实验四的 LR(0) 规范族与 FOLLOW 集生成（在本实验代码中自动构造）。
	\item Token 流：来自实验二扫描器输出（如 \texttt{tokens.out}），包含类型与词素。
\end{itemize}
预处理的关键步骤：
\begin{itemize}
	\item 将 Token 类型映射到文法终结符（例如 \texttt{ID -> id}、\texttt{INT/\# -> num}）。
	\item 为文法自动增广起始产生式，构造 LR(0) 项目集和 SLR(1) ACTION/GOTO 表。
\end{itemize}

\subsection{实现步骤}
\begin{enumerate}
	\item \textbf{文法扩展与语义动作设计}：为函数、语句、表达式等产生式设计对应语义动作。
	\item \textbf{属性栈与语法栈联动}：在规约时弹出右部属性并构造父节点/类型信息。
	\item \textbf{符号表与作用域维护}：进入/离开代码块时更新作用域，记录重复声明与未声明错误。
	\item \textbf{输出与验证}：打印 AST、符号表和语义错误列表。
\end{enumerate}

\section{实验内容}
\subsection{任务一：SLR 引导的语义分析框架}
本实验采用 ``\texttt{SLR(1) 分析 + 属性栈}'' 结构：语法分析使用 ACTION/GOTO 表驱动，
每次规约触发语义动作，构建 AST 并传递类型信息。文法包含函数定义、参数、声明、赋值、条件、循环与表达式。
...
Cond -> Expr rop Expr
\end{verbatim}
\textbf{说明}：该片段展示了本实验支持的关键语句结构，包含函数、声明、赋值与控制流，便于与 Token 流对齐。

\subsection{任务二：AST 构建与语义检查}
核心语义动作包括：
\begin{itemize}
	\item 声明语句：遍历声明列表，将变量加入符号表，检查重复声明。
	\item 赋值语句：检查左值是否声明，比较左右值类型并报告不匹配。
	\item 条件表达式：使用关系运算符生成条件节点，类型统一为整型逻辑值。
\end{itemize}

\textbf{代码片段（声明与赋值语义动作节选）}：
\begin{lstlisting}[language=C]
case 19: /* Decl -> Type DeclList ; */
	result.node = ast_create("decl", value_type_name(rhs_attrs[0].type));
...
case 22: /* Assign -> id = Expr ; */
	{
		...
		if (!lookup_symbol(name, &lhs_type)) {
			record_error("Undeclared identifier");
			lhs_type = TYPE_ERROR;
		}
		if (lhs_type == TYPE_INT && rhs_attrs[2].type == TYPE_FLOAT) {
			record_error("Type mismatch");
		}
	...
	break;
\end{lstlisting}
\textbf{说明}：声明语义动作将标识符加入符号表；赋值语义动作检查类型一致性并构建 ``assign'' AST 节点。
...
\label{fig:compile}
\end{figure}
\textbf{说明}：图 \ref{fig:compile} 展示了在实验目录下编译并运行语义分析程序的过程，
验证了程序能够在给定 Token 流上完成语法与语义分析。

\subsection{AST 与语义输出}
以下为实际运行输出（节选）：
\begin{verbatim}
AST:
program
...
None
\end{verbatim}
\textbf{说明}：AST 显示了函数 ``qsort'' 的参数、块结构、条件与循环嵌套关系；
符号表记录了参数与局部变量的类型与作用域；语义错误列表为空，说明该输入在本规则下通过了检查。
...
\end{tabular}
\end{table}
\textbf{说明}：该表格对应符号表输出，作用域 0 表示函数参数，作用域 1 表示函数体内部的局部变量。

\section{总结}
本实验成功完成了 SLR 引导的语义分析框架：
\begin{itemize}
	\item 基于 SLR(1) 分析表完成语法驱动的语义动作执行。
	\item 构建了表达式与语句层面的 AST，支持函数、控制流与块结构。
	\item 实现符号表与作用域管理，支持重复声明、未声明与类型不匹配检查。
\end{itemize}
后续可扩展：增加函数返回类型检查、参数数量/类型匹配与更丰富的语句类型。
...
\begin{verbatim}
# 编译
gcc -Wall -Wextra -o semantic main.c

# 运行
./semantic /path/to/tokens.out
\end{verbatim}
