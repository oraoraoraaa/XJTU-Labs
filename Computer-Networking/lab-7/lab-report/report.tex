...
% XeLaTeX 中文支持（适用于 Overleaf）
\usepackage{fontspec}
\usepackage{xeCJK}
% 为避免交互式提示与字体不可用问题，直接使用 Overleaf 常见的 Source Han 字体族。
% 如果你的 Overleaf 模板缺少该字体，请在项目设置中切换为 XeLaTeX，并尝试下列替代字体之一：
% 'Noto Serif CJK SC', 'Noto Sans CJK SC', 'Source Han Serif SC', 'Source Han Sans SC', 'SimSun'
...
if car is None:
    ...
    c.execute("""
    UPDATE cars
    SET exit_time=?, fee=?, status='OUT'
    WHERE plate=? AND status='在场'
    """, (str(now), fee, plate))
\end{lstlisting}

数据库表\texttt{cars}在\texttt{init\_db()}中创建，字段依次是车牌、入场时间、出场时间、费用和状态。入场时写入一条状态为“在场”的记录；再次识别到同一车牌时，系统把它视为出场请求，读取最早的入场时间，按\texttt{(now - enter\_time).total\_seconds() / 3600}换算出停车小时数，再乘以每小时5元得到费用。

这里的状态设计比较简洁，但足以构成一个最小闭环：只有“在场”状态才允许进入出场结算分支，避免重复扣费或重复写出场信息。也就是说，业务规则不是依赖前端按钮，而是由数据库中的状态字段来保证。
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{images/new_error1_term.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{images/new_error2_term.png}
