...
def get_img_dataloaders(base_dir):
    ...
    train_dataset = FaceDataset(
        data_dir, "train", img_size=128, augment=True, age_stddev=AGE_STDDEV
    )
    train_loader = DataLoader(
        train_dataset,
        batch_size=BATCH_SIZE,
...
class FaceDataset(Dataset):
    def __init__(
        self, data_dir, data_type, img_size=224, augment=False, age_stddev=1.0
    ):
        ...
        for _, row in df.iterrows():
            ...
            if not os.path.exists(img_path):
                print(f"Missing file path: {img_path}")
